...
$$\begin{align*}\\\text{A : Adjecency matrix,}\ \ A_{uv} = 1\ if\ u\in N(v)\\c : \text{Centrality vector}\\ \lambda: \text{Eigenvalue}\end{align*}$$
...
$$\begin{equation}c_v = \sum_{s\neq v \neq t}\frac{\# \text{ of shortest paht between s and t that contains v}}{\# \text{ of shortest paths between s and t}}\tag{1}\end{equation}$$
...
$$\begin{equation}\tag{1}c_v = \frac{1}{\sum_{u\neq v}\text{ shortest path length between u and v}}\end{equation}$$
...
$$\begin{equation}\tag{1}e_v = \frac{\#\text{ of edges among neighboring nodes}}{k_v\choose2} \in [0, 1]\end{equation}$$
